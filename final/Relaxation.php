...
<html>
  <head>
    <meta charset="utf-8">    
    <title>Yoga Practice: Relaxation</title>
...
function checkSession(){
  ...
  /* check for bad data */
  if(title > 20){
    document.getElementById("Output6").innerHTML = "Please enter a title that is 20 characters or less";
    document.getElementById("AddFormRelax").reset();
   return;
  }

  if(isNaN(length)){
    document.getElementById("Output6").innerHTML ="Length input should be numeric.";
    document.getElementById("AddFormRelax").reset();
    return;
  }
    ...
    /* response was true, add the message */
     if(req.responseText == 1){
     /* update table*/
      window.location.href = "http://web.engr.oregonstate.edu/~masseyta/final/main.php";
     }

     /* response was false, tell the user */
    if(req.responseText == 0){
      document.getElementById("Output6").innerHTML = "Error adding practice to log.";
      document.getElementById("AddFormRelax").reset(); 
    }

    /* something totally weird I didn't think of happened */
     else if(req.responseText != 1 && req.responseText != 0){
          document.getElementById("Output6").innerHTML = req.responseText;
       }
    }
  }
  ...
  <div class="sidebar">
    ...
    echo "<p>";
      echo "<h2>Welcome to your Relaxation Practice, ".$_SESSION['username']."!</h2>";
